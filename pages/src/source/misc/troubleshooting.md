...
### CHECKLIST: Identifying why a citizen is not working
  ...
  7. Is their hunger level low? Citizens won't work while hungry. Give them food, and if you have a {% building_link restaurant %}, check that it is stocked and you have hired a {% worker_link cook %}. If they are hungry but won't eat the food they have, fire them until they eat then rehire them.
...
### CHECKLIST: Troubleshooting a citizen not working by career
  1. {% worker_link builder %}: Remember to click the Build Building button inside the hut block you want to be built, then wait for the {% building_link builder %} to announce they have begun building. If you are upgrading a building, check that the {% worker_link builder %} level is higher than the building you are upgrading.
  2. {% worker_link forester %}: Set which type of tree they are allowed to cut and if they should replant them. Make sure that your trees are at ground level, or if you place them one block higher, place slabs next to them.
  3. {% worker_link miner %}: Is there an ore they cannot mine in their way? Break it for them or assign them to a new mining level. Do they have cobblestone, torches, a pickaxe, a shovel, slabs, planks, and fencing (for the mineshaft)?
  4. {% worker_link cook %}: Set which fuel you want them to use.
  5. {% worker_link baker %}: Set which fuel you want them to use and which recipe you want them to bake.
  6. {% worker_link smelter %}: Set which fuel you want them to use and which ore you want them to smelt.
  7. {% worker_link farmer %}: Assign the farmer to a field. Make sure each scarecrow has a seed and make sure all tilled soil is hydrated.
  8. {% worker_link composter %}: Set which items you want them to convert into [compost](../../source/items/compost).
  9. Crafter ({% worker_link carpenter %}, {% worker_link blacksmith %}, {% worker_link mechanic %}, etc): The request for items must be made after the crafter is hired and the colony must have a {% worker_link courier %}. Cancel the request at the original NPC to generate a new request, or make a new request from the [Postbox](../../source/items/postbox). Also remember to teach them the recipes they're allowed to craft and make sure afterwards that the recipes were saved.
  10. {% worker_link stonesmelter %}: Set which fuel you want them to use. (They know their recipes already.)  
...
### CHECKLIST: Troubleshooting a Citizen not working for unknown reason
  ...
  5. Recall all citizens to the {% building_link townhall %}.
  ...
  13. Repair their work hut. Some workers will stop working if required elements are not present. Only the {% worker_link builder %} can place beds, crafting tables, furnaces, chests, [racks](../../source/items/rack), and [compost barrels](../../source/items/compostbarrel). If you broke any of those, create a repair build order for the building.
