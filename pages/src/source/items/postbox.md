...
The postbox is for the player to request items from the {% building_link warehouse %}. A {% worker_link courier %} will deliver them to the postbox.

The postbox can be crafted at the {% building_link sawmill %} or by the player.
...
There is also an option to 'Deliver what's available.' If, for example, you have this on and you request 64 stone, but your {% building_link warehouse %} only has 33 (and you don't have a {% worker_link stonesmelter %} to smelt more), a {% worker_link courier %} will bring you the 33 stone instead of none. (This option will not stay on if you exit the postbox GUI, but any requests made with it turned on will stay the same.)
