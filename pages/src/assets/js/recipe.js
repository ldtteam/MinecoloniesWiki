...
class JSONReader {
  ...
  createDiv(element, large = false, stacksize = 0, shapeless = false) {
    ...
    let recipe = this;
    output += `<div `;
    Array.isArray(element) ? large ? (output += `class="invslot invslot-large animate"`) : (output += `class="invslot animate"`) : large ? (output += `class="invslot invslot-large"`) : (output += `class="invslot"`);
    output += ` >`;
    if (element !== "") {
  ...
  contains(recipeName) {
    for (var i = 0; i < this.constructedRecipes.length; i = i + 1) {
